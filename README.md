...
Query 1 takes a quick look through the database pulls up every movie marked PG-13 or R, if it runs past ninety minutes - title, rating, duration included - with results sorted by how long they play, starting with the longest. Since organizers must plan carefully around certain screenings, knowing which ones carry these specific traits helps make decisions easier. Films like these usually land in late-night spots on the schedule so earlier, kid-friendly films stay uninterrupted. At entry points, staff might check IDs when such titles screen, just to follow rules about who can attend. Warnings also appear beside their names in printed guides, alerting parents ahead of time. Grabbing both maturity level and length together cuts down clutter; only what matters appears here. Instead of flipping through everything one by one, the team gets exactly those entries demanding several planning steps at once. That focused output trims hours off early preparation work.<img width="609" height="362" alt="Screenshot 2026-04-09 at 12 57 11 AM" src="https://github.com/user-attachments/assets/dbad6cbc-092e-4b40-af7a-eb0dee1ff3f6" />

Query 2 lists each film category alongside its standard submission fee, the number of films submitted to it, and the estimated revenue that category generated, ordered from highest to lowest revenue. This query allows festival administrators to evaluate the financial performance of each submission category. Categories with high submission counts and fees are primary revenue drivers, while categories with low submissions may indicate a need for targeted outreach or fee adjustments to attract more filmmakers. This information directly informs budgeting decisions for the following year and helps justify the operational cost of running each category, such as dedicated screening slots and category-specific marketing. 
...
Query 3 lists all customers who have made reservations during the festival, showing their name, email, phone number, and total number of reservations made, ordered from most to fewest reservations. This query allows the festival's customer relations team to identify their most engaged attendees. Customers with multiple reservations are attending several screenings and represent the festival's most loyal audience members, these individuals are strong candidates for early access offers, loyalty discounts, or VIP invitations in future years. Ordering results by reservation count makes it immediately clear who the top attendees are, allowing staff to prioritize outreach and relationship-building efforts with minimal effort. 
...
Query 4 lists the complete festival screening schedule showing the film title, venue name, room name, screening date, start time, and room capacity, ordered by date, time, and venue name. Query 4 gives operations staff a single unified master schedule covering every screening across all venues for the entire festival week. This view is essential for coordinating AV equipment setup, volunteer deployment, audience flow between screenings, and venue-specific logistics such as lobby preparation and signage. Having capacity visible alongside each screening also allows staff to quickly identify high-demand screenings that may need additional crowd management support. 
...
Query 5 lists all films that have been officially paired together, showing both film titles side by side, the type of pairing, and the date the pairing was established. The query uses a self-join on the film table through the recursive relationship, and the WHERE condition ensures each pair only appears once rather than twice as mirror rows. Query 5 allows festival programmers to quickly see all paired screenings in one clean view. Paired films are often scheduled back to back and marketed together as a single ticketed event, so having both titles visible in the same row simplifies the process of building the festival program, designing combined promotional materials, and scheduling shared screening blocks. This query gives programmers an instant overview of all pairing commitments without needing to cross-reference individual film records manually. 
...
Query 6 lists all films that received at least one nomination but did not win any award, showing the film title, country of origin, content rating, and primary director. It uses a NOT EXISTS subquery to exclude any film that has a linked winning award result. Query 6 allows the programming and communications teams to identify films that were critically recognized through nominations but ultimately went home without a win. These films represent strong candidates for honorable mention features in the post-festival program, targeted social media spotlights, and audience recommendation campaigns. Highlighting nominated-but-not-winning films helps the festival demonstrate the breadth of quality submissions it received and gives well-deserving films additional exposure they may not otherwise receive. 
...
Query 7 lists each nominated film alongside its country of origin, total number of nominations received, total number of wins, and win rate as a percentage, ordered by nominations and wins in descending order. Query 7 allows festival leadership to identify the most critically recognized films of the festival at a glance. A high nomination count signals broad jury recognition across multiple award categories, while the win rate percentage reveals which films converted nominations into actual wins most efficiently. This combined view is essential for drafting press releases, writing the festival recap, and making data-driven programming decisions about which filmmakers and producing countries to actively recruit for future festivals. 
...
Query 8 lists all volunteers assigned to more than one shift, showing their name, email, total shifts worked, number of shifts they checked in for, and an overall attendance status label of Perfect Attendance, Partial Attendance, or No Check-ins using CASE logic. Query 8 allows volunteer coordinators to evaluate reliability across their most active volunteers. Volunteers with perfect attendance across multiple shifts are strong candidates for recognition programs, leadership roles, or priority invitations to return next year. Conversely, volunteers flagged as No Check-ins despite multiple assignments representing a scheduling and resource gap. Coordinators can follow up with these individuals to understand what went wrong and adjust future assignment policies to avoid understaffed shifts during the festival. 
...
Query 9 lists each venue alongside its address, total number of screenings held, total seats reserved across all its screenings, and the number of unique customers who made reservations, filtered to only include venues with more than 10 total seats reserved and ordered by total seats reserved. Query 9 allows the festival board to evaluate which venues drove the most audience engagement throughout the week. Venues with high seat reservation counts and unique customer counts are clearly the most popular and should be prioritized in future venue contract negotiations, capacity upgrade discussions, and marketing budget allocations. This data also helps identify underperforming venues that may not be worth renewing contracts for, or that may need targeted promotional efforts to increase attendance in future festival years.

Query 10 lists every award winner showing the award name, award category, winning film title, country of origin, primary director name, submitter name, and any result notes, ordered by award category and award name. A subquery in the WHERE clause confirms each winning film appears in the nomination table before being included in results. Query 10 produces the definitive awards winners summary that the festival needs for multiple high-visibility deliverables. Press releases, award certificates, closing night ceremony scripts, and the official festival program all require the same core information: which film won, who 
directed it, and who submitted it, and it needs to be presented in a clean, organized format. By grouping results by award category and name, this query outputs data that can be directly formatted into the ceremony program with minimal manual rework, saving significant administrative time on one of the most critical deliverables of the entire festival.
